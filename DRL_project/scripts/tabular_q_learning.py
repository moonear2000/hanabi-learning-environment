...
class Trainer(object):
  ...
  def __init__(self, flags):
    ...
    self.filename = os.path.join(dirname, 'tables/q_table_TerminalRewardOn5Only.pickle')
    try:
  ...
  def train(self):
    ...
    self.explore_phase(num_iters=1)
    # Evaluate
    print('Evaluating')
    self.evaluate(num_games=10000)
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        if episode%200000 == 0 and episode != 0:
            print('Saving table')
            self.evaluate()
            try:
                with open(self.filename, 'wb') as fp:
